module radiation
! changes: use DUMMY instead of CO2 in LW and SW calculation
...
! added DUMMY as an output variable:
    call addfld ('DUMMY   ','mol/mol ',1,    'I','dummy variable',phys_decomp)
...
! added mwdry and mwco2 here
    use physconst,        only: cpair, stebol, mwdry, mwco2
    ...
    real(r8), pointer, dimension(:,:) :: dummy  ! dummy variable mass mixing ratio
...
!----------------------------------------------------------------------
    ...
    if (dosw .or. dolw) then
       ...
       ! Get dummy variable mass mixing ratio.
       call rad_cnst_get_gas(0,'DUMMY',  state, pbuf,  dummy)
...
! replaced co2_col_mean by dummy
          call radcswmx(lchnk, &
               ...
               Nday,       Nnite,      IdxDay,     IdxNite,    dummy, &
          ...
          call outfld('DUMMY   ',dummy,pcols,lchnk)
